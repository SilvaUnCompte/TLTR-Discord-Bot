...
- PM2 est installé **globalement** (`npm i -g pm2`), plus en dépendance du projet
...
Le bot demande Node 20 minimum (`node -v` pour vérifier).

Trois cas où il faut faire un peu plus :

| Situation                                   | Commande en plus                                              |
| ------------------------------------------- | ------------------------------------------------------------- |
| Les slash commands ont changé (`commands/`) | `npm run deploy-commands` **avant** le restart                |
...
| `Missing required environment variable` | `.env` absent ou illisible                                                     |
